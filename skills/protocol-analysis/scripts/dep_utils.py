...
def annotate_survey_with_dependencies(survey: list, form: dict = None) -> list:
    """
    Add a 'dependencies' key to each survey row containing the
    list of [FormOID].[ItemOID] references that row needs.

    Sources:
      1. XPath expressions in the row itself (calculation, constraint, relevant).
      2. The form's cross_form_dependencies array — a declared catalog of
         cross-form pulls. Each entry is attributed to the survey row whose
         name matches the dep's target_field (or, absent target_field, the
         source_field).

    The `form` argument is optional for backward compatibility; pass the
    full form dict to enable source-2 attribution.
    """
    # Build a map from row `name` to declared deps it should show.
    # Strategy: if a dep entry has `target_field`, attribute to the row
    # whose name matches that. Otherwise attribute to the row whose name
    # matches the source_field (typical for SUBJID pulled into every form).
    row_level_decl = {}
    if form is not None:
        for dep in form.get("cross_form_dependencies", []):
            src_form  = dep.get("source_form", "")
            src_field = dep.get("source_field", "")
            tgt_field = dep.get("target_field") or src_field
            oid = dep.get("source_item_oid") or (
                f"{src_form}.{src_field}" if src_form and src_field else ""
            )
            if not oid or not tgt_field:
                continue
            row_level_decl.setdefault(tgt_field, set()).add(oid)

    for row in survey:
        deps_set = set(extract_row_dependencies(row))
        name = row.get("name", "")
        if name and name in row_level_decl:
            deps_set |= row_level_decl[name]
        row["dependencies"] = sorted(deps_set)
    return survey
