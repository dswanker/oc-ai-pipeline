...
def build_edc_pdf(data: dict, output_path: str):
    ...
    header_data = [[
        Paragraph("EDC STRUCTURE SUMMARY", styles["title"]),
        Paragraph(
    ...
    inv_headers = ["#", "Form ID", "Form Title", "Category", "CDASH", "Repeating",
    ...
    for i, form in enumerate(forms, 1):
        ...
        # Settings + meta row
        meta_left = [
            ("Form ID",       settings.get("form_id", form_id)),
        ...
        meta_right = [
            ...
            ("Survey Rows",   f"{len(survey)} total  ({n_complete} complete / {n_flagged} flagged / {n_placeholder} placeholder)"),
        ...
        if survey:
            annotate_survey_with_dependencies(survey)
            s_headers = ["type", "name", "label", "itemgroup", "appearance",
                         "relevant", "required", "constraint", "calculation",
                         "dependencies", "status"]
            s_cw = [
                CONTENT_W*0.08, CONTENT_W*0.08, CONTENT_W*0.12, CONTENT_W*0.05,
            ...
            for row in show_rows:
                ...
                s_data.append([
                    ...
                    Paragraph(str(row.get("bind__oc_itemgroup","") or "")[:10], styles["cell"]),
    ...
    if all_deps:
        dep_headers = ["Source Form", "Source Field", "Target Form", "Purpose", "Visit Context", "Status"]
        dep_cw = [CONTENT_W*0.10, CONTENT_W*0.12, CONTENT_W*0.10,
                  CONTENT_W*0.28, CONTENT_W*0.15, CONTENT_W*0.25]
        dep_data = []
        for d in all_deps:
            dep_data.append([
                d.get("source_form",""),
