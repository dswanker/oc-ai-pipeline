...
def build_single_xlsform(form_data, output_path, build_log):
    ...
    # ── Inject begin_group / end_group for non-repeating forms ────────────
    # OC4's ODM import engine resolves ItemGroup OIDs from the form metadata
    # registered at upload time. OC only registers an ItemGroup (e.g. IG_VS_VS)
    # if the XLSForm contains a begin_group row whose 'name' matches the
    # bind::oc:itemgroup value. Without begin_group, ODM import returns
    # errorCode.itemGroupOIDNotFound even though the form uploads and renders
    # correctly in Enketo (which uses bind::oc:itemgroup natively).
    # Fix: for non-repeating forms that have bind::oc:itemgroup but no
    # begin_group row, inject a wrapper begin_group/end_group around the
    # data fields so OC registers the ItemGroup in its metadata.
    _BLOCK_TYPES_CHECK = {
        'begin group', 'begin repeat', 'end group', 'end repeat',
        'begin_group', 'begin_repeat', 'end_group', 'end_repeat',
    }
    _is_repeating_form = bool(
        _repeat_group_name
        or str(form_data.get('repeating', '') or '').strip().lower()
        not in ('no', 'false', '0', '')
    )
    _has_begin_group = any(
        str(r.get('type', '') or '').strip().lower() in ('begin group', 'begin_group')
        for r in survey
    )
    if not _is_repeating_form and not _has_begin_group:
        # Find the most common bind::oc:itemgroup value to use as group name
        _ig_vals = [
            str(r.get('bind::oc:itemgroup') or '').strip()
            for r in survey
            if str(r.get('type', '') or '').strip().lower()
               not in _BLOCK_TYPES_CHECK
            and str(r.get('bind::oc:itemgroup') or '').strip()
        ]
        if _ig_vals:
            from collections import Counter as _Ctr
            _grp_name = _Ctr(_ig_vals).most_common(1)[0][0]
            # Wrap data rows (non-calculate, non-note) in begin_group / end_group.
            # Calculates that precede the group (external lookups with no
            # itemgroup) stay outside the group.
            _DATA_TYPES_FOR_WRAP = {
                'text','integer','decimal','date','time','datetime',
                'select_one','select_multiple','geopoint','geotrace',
                'geoshape','range','file','image','audio','video','barcode',
            }
            _before, _inside, _after = [], [], []
            _in_group = False
            for _r in survey:
                _rt = str(_r.get('type', '') or '').strip().lower()
                _has_ig = bool(str(_r.get('bind::oc:itemgroup') or '').strip())
                if not _in_group and (_has_ig or any(_rt.startswith(dt) for dt in _DATA_TYPES_FOR_WRAP)):
                    _in_group = True
                if _in_group:
                    _inside.append(_r)
                else:
            ...
            survey = (
                _before
                + [{'type': 'begin group', 'name': _grp_name, 'label': _grp_name, 'appearance': 'field-list'}]
                + _inside
                + [{'type': 'end group', 'name': ''}]
            )
            print(f"[edc-builder] {form_id}: injected begin_group name={_grp_name!r} "
                  f"({len(_inside)} rows inside — registers IG_{form_id}_{_grp_name} in OC metadata)",
                  flush=True)
