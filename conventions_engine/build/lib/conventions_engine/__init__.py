...
def _default_data_root() -> Path:
    """Conventions data ships with the engine as package data.

    The data tree lives at <package>/conventions/ (i.e., as a
    subdirectory of conventions_engine/). At runtime we return the
    package directory itself, and the loader appends "conventions"
    to find schema/, global/, vendors/.

    This used to live at the repo root as a sibling of the package
    (<repo>/conventions/), and the function was named
    `_default_repo_root`. The data was moved inside the package so
    pip-installed copies of the engine work without needing a sibling
    directory (see Option A refactor)."""
