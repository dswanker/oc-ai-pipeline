    ...
    for fname, data in _expanded_cho:
        ...
        try:
            ...
            for row in reader:
                ...
                if not var_safe:
                    continue  # skip entirely invalid names

                # Skip iMedNet UI artifacts and form-designer annotations
                # These are internal iMedNet controls, not real data fields
                _SKIP_PATTERNS = [
                    'generated by form designer',
                    'hide + clear', 'hide+clear', 'hide_clear',
                    'display message', 'display_message',
                    'always disable', 'always_disable',
                    'edit - ', 'edit_-_',
                ]
                _label_lower = (label or var).lower()
                _var_lower = var.lower()
                _skip = any(p in _label_lower or p in _var_lower
                            for p in _SKIP_PATTERNS)
                # Also skip if label looks like a form designer annotation
                if not _skip and label and (
                    label.startswith('Generated by') or
                    label.startswith('Edit -') or
                    label in ('Hard', 'D+C', 'Hide + Clear')
                ):
                    _skip = True
                if _skip:
                    print(f"[crf-standards] Skipping UI artifact: {form}.{var} ({label!r})",
                          flush=True)
                    continue
...
def _apply_crf_standards(struct_json: dict, crf_files: list, oc_files: list) -> dict:
    ...
    for form in struct_json.get("forms", []):
        ...
        for field in required_fields:
            ...
            # Use variable name directly — no I_FORM_ prefix.
            # Injected fields must match the naming convention Claude uses
            # (bare variable names like RACE1, AETERM) not iMedNet OID format.
            full_name = var_name_safe
            new_row = {
                "type":                 xls_type_str,
                "name":                 full_name,
                "label":                field["label"],
