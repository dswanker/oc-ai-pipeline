...
from customer_conventions_client import (
    extract_customer_conventions,
    save_customer_conventions,
    load_rulebook_conventions,
    merge_conventions,
...
async def run_pipeline(item_id):
    try:
        ...
        # ── Customer conventions extraction ──────────────────────────────────
        # Reads all CQ_* columns dynamically. New questions added to the monday
        # board (any column with title or id starting with "CQ_") are picked
        # up automatically — no code change required.
        # Customer answers supersede rulebook conventions at merge time.
        customer_conv = extract_customer_conventions(cols)
        if customer_conv:
            save_customer_conventions(item_id, customer_conv)
            print(f"CUSTOMER CONVENTIONS: {len(customer_conv)} answer(s) provided",
                  flush=True)
        else:
            print("CUSTOMER CONVENTIONS: none provided", flush=True)

        rulebook = load_rulebook_conventions()
        merged_conv = merge_conventions(rulebook, customer_conv)
        conventions_prompt_block = build_conventions_prompt_block(merged_conv)
        if conventions_prompt_block:
            await append_log(
                item_id,
                f"Conventions applied — customer:{len(customer_conv)} "
                f"rulebook:{len(rulebook)} effective:{len(merged_conv['effective'])}"
            )
        ...
        struct_response = await run_skill(
            ...
            extra_text = (
                (conventions_prompt_block + "\n" if conventions_prompt_block else "")
                + ("Customer CRF library attached." if crf_pdf else "")
            )
        )
        ...
        build_response = await run_skill(
            EDC_BUILD_PROMPT,
            xlsx_bytes=spec_xlsx,
            extra_text=(conventions_prompt_block if conventions_prompt_block else ""),
        )
